...
export default function Project({ projectId }: { projectId: string }) {
  ...
  const { mutate: createTestSuite } = useCreateTestSuite(projectId);
  ...
  return 	<div key={projectId as string} className="flex h-full">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
            <div className="flex items-center gap-1">
              ...
              <span>{(project?.data as any)?._count?.testSuites} test suites</span>
            </div>
            <div className="flex items-center gap-1">
              <CheckCircle className="h-4 w-4 text-green-500" />
              <span>
                {(project?.data as any)?.testSuites?.reduce((total: number, suite: any) => 
                  total + (suite._count?.testCases || 0), 0
                ) || 0} test cases
              </span>
            </div>
            <div className="flex items-center gap-1">
