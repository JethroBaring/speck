...
import { useRunTestSuite, useTestSuite } from "@/hooks/useTestSuites";
...
import { useTestRunnerWebSocket } from "@/hooks/useTestRunnerWebsocket";
...
export default function TestSuite({ testSuiteId }: { testSuiteId: string; }) {
	...
	const {
		...
		cancelTestCase,
		retryTestCase,
	} = useTestRunnerWebSocket(currentTestSuiteRunId || '');
	...
	const handleRunTestSuite = () => {
		runTestSuite(undefined, {
			onSuccess: (response) => {
				toast.addToast({
					title: "Test suite run started",
					message: "Test suite run started",
					type: "success",
				});
				console.log("response?.data?.testSuiteRunId", response?.data?.testSuiteRunId);
	...
	// Connection is managed by useTestRunnerWebSocket via currentTestSuiteRunId
	...
	return (
		<>
			<div className="flex h-full">
				...
				<TestSuiteRightSidebar projectId={projectId as string} testSuiteId={testSuiteId} />
			</div>
